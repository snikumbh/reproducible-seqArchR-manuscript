...
nucl_df <- data.frame(x0 = c(100, 110, 120), 
                      y0 = rep(5, 3),
                      r = rep(4.5, 3))
...
pl <- ggplot() + geom_blank() + xlim(-50, 100) + ylim(0, 50) + 
      ...
      ggplot2::annotate(geom = "segment", 
          x = c(100, 110, 120, 95.5, 105.5, 115.5), xend = c(100, 110, 120, 104.5, 114.5, 124.5), 
          y = c(rep(0.5, 3), rep(5, 3)), yend = c(rep(9.5, 3), rep(5, 3)), alpha = 0.2) +
      ## 
      ggplot2::coord_fixed() +
      ## TATA
      motifStack::geom_motif(data = motifs_df, 
...
The GO terms enriched for these clusters are quite similar 
(Figure \@ref(fig:go-terms-comparison-dm)E and Supplementary 
Figure 10).
...
<!-- Among the elements orchestrating focused transcription, namely the `TATA`-box  -->
<!-- and DPE, the `TATA`-box is not observed until 10-12h AEL. -->
<!-- (2-4h) -->
<!-- which are  -->
...
which are present in hundreds of moderately divergent copies [@yavor2023mir], 
...
fertilisation \textemdash\space  
positive regulation of cellular component organisation 
and negative regulation of fertilisation (Supplementary Figure 15).
