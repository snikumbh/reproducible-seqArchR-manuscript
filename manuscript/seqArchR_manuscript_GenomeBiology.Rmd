...
author: 
 ...
 - "Boris Lenhard^[MRC London Institute of Medical Sciences and Imperial College London, W12 0NN UK, b.lenhard@imperial.ac.uk]"
...
Animal promoters come in different classes characterised by enrichment of a 
variety of features in the promoter sequence.
The consequent inherent diversity of mechanisms for transcriptional regulation 
of genes is still poorly understood.
This is in part due to lack of comprehensive, *de novo* characterisation of 
these promoter sequence architectures.
Here, we present seqArchR, an unsupervised learning approach using 
non-negative matrix factorisation for *de novo* identification of diverse 
promoter sequence architectures.
seqArchR is available as an R/Bioconductor package and can be accessed 
at: [http://www.bioconductor.org/packages/seqArchR](http://www.bioconductor.org/packages/seqArchR).
...
archR_hg_results_path <- file.path(results_path,
                                "human", "newly_processed", "minTPM_1")
...
Promoter sequences are portions of DNA flanking the transcription start site 
(TSS) of a gene.
...
For instance, \texttt{TATA}-box, which binds the \texttt{TATA}-box binding 
...
Sharp initiation is associated with tissue-specific promoters, while broad 
promoters are found with constitutively expressed promoters and a subset of 
...
```{r archR-schematic-bio, fig.align="center", out.width = "140%", out.height="30%", echo=FALSE, include=TRUE, fig.cap="Schema depicting how across organisms, different promoter architectures have a TSS-selection determining sequence element at near-fixed distance from their dominant transcription start sites."}


use_fname <- file.path(figures_path, 
                    "seqArchR-figure1-bio-schematic-crop.pdf")
...
algorithms and identify architectures in promoter sequences *de novo* and  are 
...
NPLB has a relatively long execution time.
...
from ChIP-seq like data, it is unimodal and does not cater to the scenario of 
multiple modes of protein-DNA binding [@mumodLeelavati].
...
identification of sequence architectures.
...
architectures, and along its rows are the loadings for the features per 
architecture (see Figure \@ref(fig:archR-algo-figure), panel B).
...
architecture.
A sequence with a relatively high coefficient for an architecture is expected 
...
basis vectors
(see \@ref(methods) for additional details). 
...
The algorithm is described in detail in Section \@ref(methods).
...
filenames <- list.files(path = "/mnt/biggley/home/sarvesh/projects/promoter-architectures-all/clustering-promoter-architectures/promArch_only_for_biorxiv_manuscript/promArch/manuscript/biorxiv/experiments/results/simulated-data-archR-v0.1.8-bound-1e08/48cores",
...
# filename <- file.path("/mnt/biggley/home/sarvesh/projects/promoter-architectures-all/clustering-promoter-architectures/promArch_only_for_biorxiv_manuscript/promArch/manuscript/biorxiv/experiments/results/simulated-data-archR-v0.1.8-bound-1e08/48cores", "archR_result_simulated_data_summary_mem2.tsv")
# archR_simdata_results_mem1 <- readr::read_tsv(file = filename, col_names = TRUE, 
#                                         col_types = cols())


# filename <- file.path("/mnt/biggley/home/sarvesh/projects/promoter-architectures-all/clustering-promoter-architectures/promArch_only_for_biorxiv_manuscript/promArch/manuscript/biorxiv/experiments/results/simulated-data-archR-v0.1.8-bound-1e08/48cores", "archR_result_simulated_data_summary_mem3.tsv")
# archR_simdata_results_mem2 <- readr::read_tsv(file = filename, col_names = TRUE, 
#                                         col_types = cols())
# 
# archR_simdata_results_mem <- rbind(archR_simdata_results_mem1, archR_simdata_results_mem2)
...
In Figure \@ref(fig:simdata-results)E, the colors denote time taken by 
seqArchR to complete different iterations.
We observe that processing the whole data in many, small chunks takes more 
time overall.
This is expected due to the following reason.
For each chunk, the stability-based model selection procedure starts from 
checking for just two clusters and continues checking further values until 
the bounding threshold on the instability of identified cluster features is 
satisfied (See more details in Section \@ref(archR-algorithm)).
...
```{r ggpubr-ari-facet-grid, fig.align="center", out.height="100%", warning=FALSE, dpi=500, fig.cap="Quantitative assessment of performance of seqArchR on simulated data. (\\textbf{A}) Adjusted Rand index (ARI) attained by seqArchR with various parameter combinations on simulated data (data described in Table \\ref{tab:table-simulated-data}). Bar heights represent average over ten runs. As described, experiments are performed for chunk sizes 200, 500, and 1000 for different mutation rates (m) and number of mutated positions in motifs (p). All experiments with simulated data were performed with parameter \\textit{bound} set to 1e-08 for seqArchR. (\\textbf{B}) ARI values attained by NPLB. (\\textbf{C}) ARI attained by seqArchR for two scaled up versions of the simulated data: 5000 and 10000 sequences. See main text for more details. (\\textbf{D}, \\textbf{E}) Time taken by seqArchR and NPLB respectively to process simulated data."}
...
# archR_ari_pl2 <- lapply(nSeqs_val, function(x){
#     ##
#     relevant_idx <- dplyr::filter(archR_simdata_results, nSeqs == x)$index
...
#     ari_melted_df2$chunkSize <- ari_melted_df2$chunkSize/1000
#     ##
#     this_plot <-  ggpubr::ggbarplot(data = ari_melted_df2, 
#                 x = "chunkSize", y = "value",
#                 fill = paired_blues[1], 
#                 size = 0.5, color = paired_blues[1],
#                 facet.by = c("nSeqs"),
#                 legend = "none",
#                 ylim = c(0.25,1.05), 
#                 xlab = ifelse(x == 10000, paste0("Chunk size ", 
#                       expression('(x 1K)')), FALSE),
#                 ylab = "ARI",
#                 ggtheme = ggpubr::theme_pubr(),#theme_bw(base_size = 12),
#                 legend.title = "Stage",
#                 ## add params
...
#                 lab.nb.digits = 2, lab.size = 3.5,
#                 ##
#                 panel.labs = list(nSeqs = 
#                         # paste("p = 1, m = 0.1, n =", c(x))),
#                         paste0("n = ", c(x)/1000, "K")),
...
colnames(copy_simdata_results)[9:10] <- paste0(1:2)#paste0("Iteration", 1:2)
#
...
# archR_time_pl <- ggpubr::facet(archR_time_pl, facet.by = "nSeqsInK")
...
# archR_time_pl_inset <- ggpubr::ggbarplot(data = time_melted_df2[relevant_idx,],
#                 x = "chunkSize", y = "value",
#                 fill = "variable", width = 0.4, color = "variable",
#                 # facet.by = c("nSeqs"),
#                 ## add params
#                 add = c("mean"),
#                 add.params = list(size= 0.3, color = paired_blues[2]),
#                 ## label params, if any
#                 ##
#                 xlab = paste0("Chunk size ", expression('(x 1K)')),
#                 #ifelse(x == 10000, "Chunk size", FALSE),
#                 ylab = "Time (minutes)", #ifelse(x == 1000, "Time (seconds)", "Time (minutes)"),
#                 # legend.title = "archR stage",
#                 # legend = "top",#  "right",
#                 # panel.labs = list(nSeqs = paste("n =", c(x))),
#                 # panel.labs = list(nSeqs = 
#                 #                     paste0("n = ",    
#                                              as.numeric(levels(as.factor(time_melted_df2$nSeqs)))/1000, 
#                 #                            "K")),
#                 ggtheme = ggpubr::theme_pubr(),# theme_bw(base_size = 12),
#                 x.text.angle = 0, palette = "Paired",
#                 scales = "free_x"
...
# archR_time_pl <- lapply(nSeqs_val, function(x){
#     ##
#     relevant_idx <- dplyr::filter(archR_simdata_results, nSeqs == x)$index
#     time_melted_df2 <- reshape2::melt(copy_simdata_results[relevant_idx,],
#                 id.vars = c('chunkSize', 'nSeqs'),
#                 measure.vars = paste0("Iteration", 2:1)
#                 # This is iter2 followed by iter1 so that stacked bar plot has 
#                 # the right order
#                 )
#     time_melted_df2$chunkSize <- time_melted_df2$chunkSize/1000
#     if(x == 1000){
#       relevant_idx <- which(time_melted_df2$nSeqs == 1000)
#       time_melted_df2$value[relevant_idx] <- time_melted_df2$value[relevant_idx]/60
#     }
#     ##
#     this_plot <- ggpubr::ggbarplot(data = time_melted_df2,
#                 x = "chunkSize", y = "value",
#                 fill = "variable", width = 0.4, color = "variable",
#                 facet.by = c("nSeqs"),
#                 ## add params
...
#                 ## label params, if any
#                 ##
#                 xlab = "Chunk size", #ifelse(x == 10000, "Chunk size", FALSE),
#                 ylab = "Time (minutes)",  #ifelse(x == 1000, "Time (seconds)", "Time (minutes)"),
#                 legend.title = "archR stage",
#                 legend = "none",#  "right",
#                 panel.labs = list(nSeqs = paste("n =", c(x))),
...
```
 ...
 <!-- out.height="480px", fig.width=8, fig.height=6, dpi=500, warning=FALSE, message=FALSE, -->

```{r simdata-results, fig.align="center", out.height="19cm",   message=FALSE, warning=FALSE, dpi=500, fig.cap="Assessment of performance of \\textit{seqArchR} on simulated data. (\\textbf{A}) Adjusted Rand index (ARI) attained by seqArchR with various parameter combinations on simulated data (data described in Table \\ref{tab:table-simulated-data}). Bar heights represent average over ten runs. Experiments are performed for varous chunk sizes, various mutation rates (m) and number of mutated motif positions (p). (\\textbf{B}) ARI values attained by NPLB. (\\textbf{C}) ARI attained by seqArchR for two scaled up versions of the simulated data: 5000 and 10000 sequences. See main text for more details. (\\textbf{D}, \\textbf{E}) Time taken by seqArchR and NPLB respectively to process simulated data (see main text for details)."}
...
```{r plot-faceted-result-ari, eval=FALSE, include=FALSE, fig.height=6, fig.width=10, dpi=300, fig.cap="Panel A shows the adjusted Rand index (ARI) attained by _seqArchR_ on simulated data variant _v1_, chunk sizes 200, 500, and 1000 bp faceted by the different mutation rates (m) and the number of mutated motif positions (p). Correspnding values for variant _v2_ are shown in panel B. Reported ARI values in the figure are for experiments performed with parameter _bound_ set to $1e-08$."}
## Old version of figure. Discarded now. **IMPORTANT**
relevant_idx <- dplyr::filter(archR_simdata_results, bound == 10^-8 &
                                nSeqs == 1000, seed == 584436111)$index


ari_melted_df <- reshape2::melt(archR_simdata_results[relevant_idx,], 
                  id.vars = c('chunkSize', 'mutpos', 'mutrate'),
                  measure.vars = c('ARI_Iter1', 'ARI_Iter2', 'ARI'))
...
mutrate_txt <- levels(as.factor(archR_simdata_results$mutrate))
mutrate.labs <- paste("m =", mutrate_txt)
names(mutrate.labs) <- mutrate_txt
# 
mutpos_txt <- levels(as.factor(archR_simdata_results$mutpos))
mutpos.labs <- paste("p =", mutpos_txt)
names(mutpos.labs) <- mutpos_txt

ari_plot <- ggplot(data = ari_melted_df,
                  mapping = aes(y = value, x = factor(chunkSize), 
                                fill = variable)) +
            theme_bw() +
            geom_col(position = "dodge", width = 0.5, color = "black", size = 0.3) +
            scale_fill_discrete(name = "Stage",
                                labels = c('Iteration 1', 'Iteration 2', 'Final')
                                ) +
            facet_grid(mutpos ~ mutrate,
                  labeller = labeller(mutrate = mutrate.labs, 
                                      mutpos = mutpos.labs)) +
            xlab("Chunk size (bp)") +
            ylab("Adjusted Rand Index (ARI)") 
...
## seqArchR seamlessly identifies fixed promoter sequence architectures in different organisms
...
[@roider2009] (**TO-DO**: perhaps, more references needed here?).
...
organisms.
All promoters of one organism do not possess the same sequence elements, but 
can harbour combinations of a few different elements.
...
@ohler2006]; in comparison, promoters of vertebrates, like zebrafish, and 
...
e.g., \texttt{CG/GC} dinucleotide content vs \texttt{A/T}-rich W-box motifs, 
...
named DMv5 (@fitzgerald2006comparative; @ohler2002computational).
...
the Inr for tissue specific architectures.
Very early in the embryogenesis, at 2-4h, we do not see any clusters with a 
`TATA`-box except for the histone gene clusters.
C6X with Inr+DPE architecture and C1Y with Inr+DPE1 architecture have a very 
short median IQW denoting highly focused transcription.
...
denoting that these start sites are not active earlier (`ADD SUPP`: See 
supplementary info/carousel for D.melanogaster).
...
```{r go-terms-comparison-dm, fig.align='center', fig.cap="Visualisation of GO terms enriched for various clusters at different developmental stages of Drosophila melanogaster. Top-5 enriched GO terms are shown for each comparison.", out.height="45%"}
...
##### Detection of recently duplicated gene families
...
Clusters C4X, C6Y, and C5Z, all clusters of His2B gene CTSSs, are corrupted 
with some non-histone ones
(See section 3.2 in the Supplementary text for more details on this corruption).
...
`T`-stretches have been reported in human genome, too, and were shown to be able 
to bind TBP (@tatapolyT).
...
The GO terms enriched for these clusters are quite similar (Supplementary 
Figure 14).
...
Elements orchestrating focused transcription, `TATA`-box and DPE, which are 
a characteristic of tissue-specific gene promoters, are not observed early on 
(2-4h). 
While the DPE is observed 6-8h onwards (C2Y and C4Z), a proper `TATA`-box only 
appears at 10-12h stage (C3Z).




<!-- #### Chen et al. 2014 {#dm-chen2014} -->
...
(CAGE) data for vertebrates, specifically zebrafish, across different stages of 
...
(early maternal), 30% epiboly/dome (late maternal), and the prim 6 stage 
(late zygotic).
...
architectures contain the canonical pyrimidine-purine initiator with a `W`-box 
upstream. 
...
```{r zf-figure-2, out.extra = 'angle=90', out.width='105%', out.height= '105%', fig.align='center', eval=TRUE, echo=FALSE, fig.cap="Clusters and architectures identified by seqArchR for \\textbf{prim6 stage} of zebrafish development. Sequence clusters arranged by the median interquantile widths (IQW) of CAGE TCs in seqArchR clusters (shortest on top, broadest at the bottom). From left to right: Box and whisker plots of per-cluster IQWs, and TPMs followed by per-cluster sequence logos, and stacked barplots showing proportion of different genomic annotations. For cluster C10Z, an additional zoomed-in view of the section from 45 bp to 150 bp upstream is shown. TPM, Tags per million."}
...
#### Motifs
In comparison to Drosophila, core promoters of other organisms do not possess 
as much variety of sequence motifs.
...
Zebrafish core promoter sequences are known to harbour the `W`-box motif, a 
weaker, `TATA`-like element, in lieu of the `TATA`-box w.r.t. position (~30 bp 
upstream of TSS) and function (directing transcription initiation from a fixed 
position). 
This is especially characteristic of maternal architectures.
...
not periodic.
In comparison, cluster C11Z, the broadest among all clusters at the prim6 stage, 
...
Cluster ZZZ (`TO-DO`: Which cluster is this?) belongs to genes encoding mir-430, 
which are present in hundreds of moderately divergent copies (`TO-DO`: 
REF ask Ferenc), 
and whose miRNA product specifically targets maternal RNAs for degradation 
[@ZFmir-430-1; @ZFmir-430-2]. 
...
Contrastingly, at the prim6 stage, many new 
...
The left group has development-related GO terms distinctively enriched in them.
Three clusters at stage prim6, C7-9Z, which are part of the left group here, 
share the fact that they have both the upstream `W`-box and downstream `W` enrichment 
missing. 
But these clusters are enriched for different GO terms (Figure \@ref(fig:zf-add-figure)B.
The GO terms exclusively enriched for cluster C8Y are related to arresting of process of 
fertilisation (`TO-DO`: better word/phrasing here?) \textemdash\space  
positive regulation of cellular component organisation 
and negative regulation of fertilisation (Supplementary Figure 13).


```{r zf-add-figure, fig.cap="\\textbf{(A)} Genomic locations of per-cluster promoters in Zebrafish developmental stages 64 cells, 30\\% epiboly/dome and prim6. \\textbf{(B)} Comparison of top-5 enriched GO terms for different clusters per stage of zebrafish development. Clusters grouped by architecture attributes", out.extra = 'angle=0', out.width="100%", fig.align='center'}
...
In the 30\% epiboly/dome stage, there are clear indications of promoter sequences 
that do not conform to the maternal architecture seen in the 64 cells stage.
By prim6 stage, about half of the promoter sequences harbour the 
...
```{r extra-fig-1, fig.align = 'center', fig.cap="Additional analyses of seqArchR clusters for \\textbf{H. sapiens}. \\textbf{(A)} Per-cluster proportions of ribosomal and other (non-ribosomal) genes (left) and proportions of dual initiation and other promoters (right). \\textbf{(B)} Proportion of housekeeping (HK) genes in each cluster with clusters arranged in ascending order of \\% HK genes (bottom) and absolute numbers (top). \\textit{seqArchR} cluster names correspond to those from the previous figure."}
...
because we expected from our analysis in (FANTOM5 paper, supplementary figure 
x `TO-DO`) that a majority of CpG-island overlapping and broad promoters will 
...
Inability to identify a more crisp `TATA` signal as a separate class is
possibly because of the dominant influence of the downstream flanking sequence 
in the chosen window.
To try to eliminate drowning of this TSS-determining motif architecture, we 
...
We reasoned that the reason for this low percentage could be due to the way 
...
protein genes (Figure \@ref(fig:extra-fig-1)). 
...
[@RPG2011basalmetazoans; @RPGyoshihama2002human; @RPG2006characteristicsHuman].
...
(see Figure \@ref(fig:extra-fig-1)) \textemdash\space proportion of dual 
...
To test the importance of the initiator towards the clustering of the 
...
`TO-DO`: Comment on the differences between mammalian genes and those of 
...
As discussed in the \@ref(methods) section, a good choice of number of 
...
for(devc in use_device){
  use_fname <- file.path("figures", devc,                          paste0("archR_manuscript_figure_simdata_memory_footprint.", devc))
...
all sequences.
For example, the canonical YR initiator \textemdash\space case of same length motifs 
overlapping in occurrence positions, and Inr `CA` overlapping with the first 
`CA` of Ohler1 motif in Drosophila. 
...
Additionally, for larger chunk-sizes, the memory usage increases linearly (see Supplementary Figure 2).
Therefore, having the ability to process very large datasets one chunk at a time enables processing them with a reasonably smaller memory footprint. 
Qualitatively, a very small chunk size can lead to a fragmented set of final clusters.


## Method Availability
seqArchR is made available as an R package with a GNU General Public License version 3 (GPL-3).
The website documenting the package is accessible at <https://snikumbh.github.io/seqArchR> and the source code at <https://github.com/snikumbh/seqArchR>.
seqArchR was previously named archR (version 0.1.8).
This is also the version used for experiments and results reported in this manuscript.
It is deposited at Zenodo with DOI [10.5281/zenodo.5055408](https://doi.org/10.5281/zenodo.5055408) [@sarvesh_nikumbh_2021_5055408].
Following version 0.1.8, there have been no changes to the algorithm.


# Acknowledgements

We acknowledge Leonie Roos for processing earlier versions of Drosophila and Zebrafish CAGE datasets.
We thank members of the Computational Regulatory Genomics group for discussions through out the project.
