...
chen2014_seqlogos_1 <- seqArchRplus::per_cluster_seqlogos(sname = "NULL", 
    ...
    clusts = seqArchR::get_seqs_clust_list(chen2014_result_serial$seqsClustLabels[[1]]), 
    bits_yax = "max", pos_lab = seq(-45,45), one_plot = TRUE, txt_size = 8)


chen2014_seqlogos_2 <- seqArchRplus::per_cluster_seqlogos(sname = "NULL", 
    seqs = chen2014_result_serial$rawSeqs, 
    clusts = seqArchR::get_seqs_clust_list(chen2014_result_serial$seqsClustLabels[[2]]), 
    bits_yax = "max", pos_lab = seq(-45,45), one_plot = TRUE, txt_size = 8)

comb_pl <- cowplot::plot_grid(chen2014_seqlogos_1, chen2014_seqlogos_2, 
                    ncol = 2, labels = "AUTO")
...
chen2014_seqlogos <- seqArchRplus::per_cluster_seqlogos(sname = "NULL", 
    seqs = chen2014_result_serial$rawSeqs, 
    clusts = seqArchR::get_seqs_clust_list(chen2014_result_serial$seqsClustLabels[[4]]), 
...
```{r go-terms-comparison-dm, fig.align='center', fig.cap="Visualisation of top-10 GO terms enriched for various clusters at different developmental stages of Drosophila melanogaster.", out.height="45%"}
...
```{r zf-result3-cluster-collation-supp-figure, fig.cap="Visualisation of the collation and curation of clusters from seqArchR raw result for prim6, D.rerio",  out.width="105%", fig.align='center'}
...
```{r zf-cluster-overlaps-supp-figure, fig.cap="Visualisation of the overlaps between promoter sequences per developmental stage for D.rerio. Stages are abbreviated as follows. 64C: 64 cells stage; D: 30\\% epiboloy/dome stage; P6: prim6 stage.",  out.width="105%", fig.align='center'}
...
lines and tissues. 
Supplementary Figure 14 shows the library sizes of various cell lines 
(including replicates) obtained from ENCODE and used upon merging together.
...
knitr::include_graphics(file.path("nonKO_short_long_combined.pdf"))
...
knitr::include_graphics(file.path("nonKO_vs_KO_long_combined.pdf"))
