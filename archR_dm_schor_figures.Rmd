...
title: "archR result analysis report for CTSS in RAL28"
output: 
...
# Sample RAL28, three developmental stages: 2-4h, 6-8h, and 10-12h AEL

This file produces additional analyses plots for sample RAL28 of D.melanogaster from Schor et al. 2017.
...
sam_foo2 <- plot_dend_arch(arch_plot = sam_foo, fname = fname2, use_ht = 60,
               ...
               new_clusts = seqs_clusters_as_list_ordered[[sample_names[sn]]],
...
## Write TATA motif sub sequences to disk
tata_seq <- dm_2017_result[[sample_names[sn]]]$rawSeqs[seqs_clusters_as_list_ordered[[sn]][[3]]]
tata_subseq <- Biostrings::subseq(tata_seq, start = 13, end = 21)
Biostrings::writeXStringSet(x = tata_subseq, 
                filepath = file.path(result_dir_path, "TATA_subsequences.fa"))


## Write DPE motif sub sequences to disk
clust_list <- seqArchR::get_seqs_clust_list(dm_2017_result[[sn]]$seqsClustLabels[[5]])
dpe_seq <- dm_2017_result[[sample_names[sn]]]$rawSeqs[clust_list[[8]]]
dpe_subseq <- Biostrings::subseq(dpe_seq, start = 72, end = 80)
Biostrings::writeXStringSet(x = dpe_subseq, 
                filepath = file.path(result_dir_path, "DPE_subsequences.fa"))
...
if(do_IQW_TPM_plots){
    ...
    for(sn in sample_names){
        ...
        pl_iqw <- get_iqw_ord_plot(iqw = TRUE, y_axis_text = TRUE, 
                                ...
                                seqs_clust = perSample_archR_clusts[[sn]], 
                                use_suffix = suffix_label[sn])
        pl_iqw <- pl_iqw + ggplot2::ggtitle(label = "Interquantile Width") + 
        ...
        pl_tpm <- get_iqw_ord_plot(tpm = TRUE, y_axis_text = FALSE,
                    ...
                    seqs_clust = perSample_archR_clusts[[sn]], 
                    use_suffix = suffix_label[sn])
        pl_tpm <- pl_tpm + ggplot2::ggtitle(label = "Expression Level") + 
            ggplot2::theme(plot.title = element_text(hjust = 0.5, 
                                                    size = use_txt_size))
        ##
        pl_phast <- get_iqw_ord_plot(phast = TRUE, y_axis_text = FALSE, 
                    ...
                    seqs_clust = perSample_archR_clusts[[sn]], 
                    use_suffix = suffix_label[sn])
        pl_phast <- pl_phast + ggplot2::ggtitle(label = "Conservation") + 
            ggplot2::theme(plot.title = element_text(hjust = 0.5, 
                                                    size = use_txt_size))
        ## IQW ordered plots
        iqw_tpm_ord_pls <- pl_iqw | pl_tpm
    ...
    for(sn in sample_names){
        ...
        function(y){
          ...
          pl <- pl + theme_classic() + 
                     ...
                     axis.text = element_text(size=use_txt_size, colour = 'black'),
                     axis.text.x = element_text(angle=0, vjust = 0.5, hjust = 0.5),
                     axis.text.y = element_text(hjust = -5),
                     # axis.text.y.left = element_text(hjust = -5), 
                     # axis.text.y.left = element_text(vjust = 0.5, hjust = 1.5), 
          ...
          ## Any annotations
          pl <- add_dm_annotation(clust_label, pl = pl, txt_size = use_txt_size)
          ##
          pl
...
for(sn in sample_names){
    ...
    bar_pl <- ggplot(overlapTypeInfo[[sn]], 
      ...
      ggplot2::ggtitle(label = 
              "Overlap between Developmental Stages") +
      # ggplot2::theme(panel.grid = element_blank(),
      #              ) + 
      # ggplot2::theme_void() + 
      # ggeasy::easy_legend_at("bottom") +
...
for(i in seq_along(show_cats)){
    p1 <- dotplot(compare_result, showCategory = show_cats[i],
                          title = paste0(arch_term, " architectures"),
...
for(i in seq_along(show_cats)){
    ...
    go_pl[[i]] <- pl
    cowplot::save_plot(
        filename = file.path(result_dir_path, 
...
arch_term <- "TATA vs DPE"
# use_color <- RColorBrewer::brewer.pal(3, "Paired")
go_term_fs <- 12
show_cats <- c(5, 10)
use_ht <- c(6, 10)
## sample/stage, clustID
clust_list <- list(c(1,6), c(2,1), c(2,8), c(3,2), c(3,3), c(3,4))
...
names(go_res) <- paste0("C", 
                        unlist(lapply(clust_list, function(x) x[2])), 
                c("X", "Y", "Y", "Z", "Z", "Z"), c(rep(" (DPE)", 4), " (TATA)", " (DPE)"))
names(go_res) <- stringr::str_wrap(names(go_res), width = 3)
...
## Polythymine stretch architectures GO term comparison
arch_term <- "Polythymine stretch"
# use_color <- RColorBrewer::brewer.pal(3, "Paired")
go_term_fs <- 12
show_cats <- c(5, 10)
use_ht <- c(6, 10)
...
names(go_res) <- paste0("C", unlist(lapply(clust_list, function(x) x[2])), 
                c("X", "Y", "Z"))
names(go_res) <- stringr::str_wrap(names(go_res), width = 3)
...
polythymine_go_pl <- go_pl



######
comb_pl2 <- cowplot::plot_grid(TataVsDpe_go_pl[[1]], tct_go_pl[[1]], ttant_go_pl[[1]], 
                    cAAA_go_pl[[1]], polythymine_go_pl[[1]], 
                    ncol = 2, labels = "AUTO", label_size = 14)

## Adding combined title
title <- ggdraw() + 
  draw_label(
    "Comparison of GO terms enriched for different promoter architectures in D. melanogaster across developmental stages",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
