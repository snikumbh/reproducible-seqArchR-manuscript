...
title: "archR result analysis report for CTSS in Human"
output: 
...
fname <- file.path(archR_org_results_path, "inrKO_5_vs_2_sankey_plot.pdf")
...
all_one_col_pl <- cowplot::plot_grid(inrKO_5_5_seqlogos, 
                                    ...
                                    ncol = 3, nrow = 1)
cowplot::save_plot(file.path(archR_org_results_path, "inrKO_5_5_vs_2_2_combined.pdf"), 
...
fname <- file.path(archR_org_results_path, "nonKO_short_long_sankey_plot.pdf")
ggsave(fname, plot = sankey_pl, device = "pdf", width = 20, height = 20)
...
all_one_col_pl <- cowplot::plot_grid(short50_5_seqlogos, 
                                    ...
                                    ncol = 3, nrow = 1)
cowplot::save_plot(file.path(archR_org_results_path, "nonKO_short_long_combined.pdf"), 
    plot = all_one_col_pl, limitsize = FALSE, 
...
fname <- file.path(archR_org_results_path, "nonKO_vs_KO_long_sankey_plot.pdf")
ggsave(fname, plot = sankey_pl, device = "pdf", width = 20, height = 20)
...
all_one_col_pl <- cowplot::plot_grid(long50_150_seqlogos, 
                                    ...
                                    ncol = 3, nrow = 1)
cowplot::save_plot(file.path(archR_org_results_path, "nonKO_vs_KO_long_combined.pdf"), 
    plot = all_one_col_pl, limitsize = FALSE, 
...
for(sn in sample_names){
    ...
    perClusterRiboProp <- lapply(seqs_clusters_as_list_ordered[[sn]], function(x){
        this_df <- new_df[x, seq(ncol(new_df))]
        riboNotRibo <- as.vector(table(this_df$isNotRibosomal))
        names(riboNotRibo) <- c("Ribosomal proteins", "Other")
        riboNotRibo
    })
    names(perClusterRiboProp) <- paste0("C", seq(length(perClusterRiboProp)))
...
perCluster_match_len <- lapply(seqs_clusters_as_list_ordered[[1]], function(x){
    ...
    names(match_len) <- c("Non-Dual Initiation", "Dual Initiation")
    match_len <- match_len
    match_len
...
percentage_pl <- ggpubr::ggbarplot(intersections, x = "cluster", y = "percentage", width = 0.5,
                                   fill = use_cols[2], ylab = "% HK genes", 
                                   xlab = "Clusters (ordered by increasing percentage)", 
    ...
    for(sn in sample_names){
        ...
        pl_tspec <- ggplot2::ggplot(samarth_df[[sn]],
            ...
            ggplot2::geom_boxplot(outlier.size = 1, width = 0.5, notch = FALSE,
                         color = "black", fill = clr[4]) +
            # scale_x_log10() +
            xlab(expression(paste(tau, " index"))) +
            ylab("Clusters") +
            ggplot2::xlim(c(0,1)) + 
            # scale_y_discrete(labels = rev(clust_labels)) +
            theme_bw() +
        ...
        pl_tspec <- pl_tspec + ggplot2::ggtitle(label = "Tissue Specificity") + 
            ggplot2::theme(plot.title = element_text(hjust = 0.5, 
                                                    size = use_txt_size))
        ##
        ## IQW ordered plots
...
for(i in seq_along(show_cats)){
    p1 <- dotplot(compare_result, showCategory = show_cats[i],
                          title = paste0("Comparing GO terms enriched for all architectures except C2 and C7-8"),
...
cowplot::save_plot(filename = file.path(result_dir_path[[sn]],
                paste0("extra_figure_human.pdf")), plot = comb_pl,
                   ncol = 2, dpi = 300, base_width = 7, base_height = 16)
